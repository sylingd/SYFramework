<?php
namespace sy\tool\wechat\pay;
use \sy\lib\YFetchURL;
use \sy\tool\wechat\Msg;
use \sy\tool\wechat\Common;
...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		$inputObj->SetAppid(Common::$APPID);//公众账号ID
		$inputObj->SetMch_id(Common::$MCHID);//商户号
	...
	public static function report($inputObj, $timeOut = 1)
	{
		...
		/*if(!$inputObj->IsInterface_urlSet()) {
			Msg::returnErrMsg(Common::, "接口URL，缺少必填参数interface_url！");
		} if(!$inputObj->IsReturn_codeSet()) {
			Msg::returnErrMsg(Common::, "返回状态码，缺少必填参数return_code！");
		} if(!$inputObj->IsResult_codeSet()) {
			Msg::returnErrMsg(Common::, "业务结果，缺少必填参数result_code！");
		} if(!$inputObj->IsUser_ipSet()) {
			Msg::returnErrMsg(Common::, "访问接口IP，缺少必填参数user_ip！");
		} if(!$inputObj->IsExecute_time_Set()) {
			Msg::returnErrMsg(Common::, "接口耗时，缺少必填参数execute_time_！");
		}*/
		$inputObj->SetAppid(Common::$APPID);//公众账号ID
		$inputObj->SetMch_id(Common::$MCHID);//商户号
